...
## Plot
For the final portion of this homework, please either use the dataset you 
  loaded, or pick a new dataset from base R to produce a `pairs()` plot 
  (scatterplot matrix). To see the datasets available in R, type `data()` at the 
  command prompt. You should see a summary of many different datasets. To get 
  information on any one of these datasets, type `?` and the name of the 
  dataset. For example `?ChickWeight` will tell you some information about the 
  dataset above. If you choose to use a base R dataset, please use one other
  than the ChickWeight dataset.
...
Once you have fulfilled all the requirements of the homework, you  need to 
  render it to html (recommended) or pdf using either the `knit` function from
  the `knitr` package, or `render` from the `rmarkdown` package. The `knit2html`
  button in Rstudio will essentially just run the code for you.
